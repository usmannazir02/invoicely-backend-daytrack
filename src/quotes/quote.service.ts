...
export class QuoteService {
    ...
    async update(id: string, updateQuoteDto: UpdateQuoteDto, user: User): Promise<Quote> {
        ...
        if (updateQuoteDto.items) {
            ...
            const discountPercentage = updateQuoteDto.discountPercentage ?? quote.discountPercentage;
            const discountAmount = (totalAmount * discountPercentage) / 100;
            const finalAmount = totalAmount - discountAmount;
            ...
            Object.assign(quote, {
                ...
                finalAmount,
                items: items as QuoteItem[],
                validUntil: updateQuoteDto.validUntil ? new Date(updateQuoteDto.validUntil) : quote.validUntil,
            });
        } else {
            // Update without recalculating
            if (updateQuoteDto.discountPercentage !== undefined) {
                const discountAmount = (quote.totalAmount * updateQuoteDto.discountPercentage) / 100;
    ...
    async updateStatus(id: string, status: QuoteStatus, user: User): Promise<Quote> {
        const quote = await this.findOne(id, user);
        quote.status = status;
        return this.quoteRepository.create(quote) as Promise<Quote>;
    }
