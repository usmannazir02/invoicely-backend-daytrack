...
export class InverterService {
    ...
    async create(createInverterDto: CreateInverterDto): Promise<Inverter> {
        ...
        return this.inverterRepository.createEntity(createInverterDto) as Promise<Inverter>;
    }

    async findAll(filterDto: FilterInverterDto) {
        const { brandId, model, minPrice, maxPrice, isActive, page = 1, limit = 10 } = filterDto;
        const where: any = {};
    ...
    async findOne(id: string): Promise<Inverter> {
        return this.inverterRepository.findOne({ id, isActive: true }, ['brand']);
    }

    async findByBrand(brandId: string): Promise<Inverter[]> {
        const result = await this.inverterRepository.find({ brandId, isActive: true }, undefined, undefined, ['brand']);
        return result.data;
    }

    async update(id: string, updateInverterDto: UpdateInverterDto): Promise<Inverter> {
        ...
        const updateData = Object.fromEntries(
            Object.entries(updateInverterDto).filter(([_, value]) => value !== undefined)
        );

        return this.inverterRepository.findOneAndUpdate({ id }, updateData) as Promise<Inverter>;
    }
