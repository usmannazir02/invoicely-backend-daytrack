...
export class StructureController {
    ...
    @Post()
    async create(@Body() createStructureDto: CreateStructureDto, @Res() res: Response) {
        const structure = await this.structureService.create(createStructureDto);
        return generateResponse(structure, 'Structure created successfully', res, 201);
    }
    ...
    async findByType(@Param('type') type: StructureType, @Res() res: Response) {
        ...
        return generateResponse(structures, 'Structures retrieved successfully', res);
    }
    ...
    async update(
        ...
        const structure = await this.structureService.update(id, updateStructureDto);
