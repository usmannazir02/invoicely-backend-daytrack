...
export class SolarPanelController {
  ...
  @Post()
  async create(
    @Body() createSolarPanelDto: CreateSolarPanelDto,
    @Res() res: Response,
  ) {
    const solarPanel = await this.solarPanelService.create(createSolarPanelDto);
    return generateResponse(
      solarPanel,
      'Solar Panel created successfully',
      res,
      201,
    );
  }
  ...
  async findByBrand(@Param('brandId') brandId: string, @Res() res: Response) {
    ...
    return generateResponse(
      solarPanels,
      'Solar Panels retrieved successfully',
      res,
    );
  }
  ...
  async findOne(@Param('id') id: string, @Res() res: Response) {
    ...
    return generateResponse(
      solarPanel,
      'Solar Panel retrieved successfully',
      res,
    );
  }
  ...
  async update(
    ...
    const solarPanel = await this.solarPanelService.update(
      id,
      updateSolarPanelDto,
    );
    return generateResponse(
